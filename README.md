...
왼쪽 목록이 접히는 **좁은 화면에서는 제목 줄의 대화방 선택기**로 오갑니다.
...
| `tests/verify.mjs` | 실제 Chromium으로 가져오기·병합·내보내기·뷰어 파일·기기 캐시·대화방 삭제·사진 배치·성능 **75개 항목** |

빠른 두 가지를 먼저 돌리므로, 해시를 잊었거나 마크업이 깨졌으면 브라우저가 뜨기 전에 멈춥니다.

**CI**: `.github/workflows/verify.yml`이 `main`에 푸시할 때마다 같은 순서로 실행하고,
통과해야만 배포합니다(PR을 쓰는 예외적인 경우에도 같은 검사가 돕니다).
...
사이드바 아래의 `뷰어 파일 내려받기`(좁은 화면에서는 `대화 도구` 패널)를 누르면 **대화가 들어 있지
않은 빈 `대화서랍.html` 한 장**을 내려받습니다. USB나 PC에 두고 더블클릭해서 열면 사이트에
들어가지 않고도, 인터넷이 끊겨 있어도 그대로 씁니다.

- 내려받은 파일에는 **그때 열려 있던 대화가 들어가지 않습니다.** 방 이름도, 보낸이 목록도
  남지 않습니다(테스트가 매번 확인합니다).
- 그 파일로 가져온 대화는 **파일이 아니라 그 브라우저의 저장소**에 남습니다. 파일을 다시 열면
  이어서 보이고, 파일을 남에게 주어도 대화는 따라가지 않습니다.
- 대화까지 담아 옮기려면 `HTML 파일로 저장`(내보내기)을 쓰세요. 이쪽은 대화가 파일 안에
  들어갑니다.
- `file://`로 열면 브라우저에 따라 저장이 막힐 수 있습니다. 그때는 `이번 화면에만 반영했습니다`
  안내가 뜨며, 닫기 전에 `HTML 파일로 저장`으로 남겨야 합니다.
