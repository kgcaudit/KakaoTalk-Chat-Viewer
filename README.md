...
여러 대화방을 한 번에 담습니다. 대화방마다 폴더가 나뉜 ZIP(카카오톡이 여러 방을 함께
내보낼 때 만드는 형태)을 그대로 넣으면 왼쪽 목록에 방이 하나씩 잡히고, 눌러서 오갑니다.
...
- **가져오기는 기존 기록을 대체합니다.** 방을 덧붙이지 않습니다. 여러 방을 함께 보려면
  그 방들이 한 ZIP에 들어 있어야 합니다(확인 창에도 같은 안내가 뜹니다).
